...
已合入团队成员的 A 第一部分，实现 Lexer、Parser 和前端调试入口；结合本地 B，
五类语句已通过 **SQL → Token → AST → 语义分析 → 逻辑计划** 联调。
现已增加 B 的规则优化：安全常量折叠、布尔化简、恒真 Filter 消除，并提供前后计划对照。
执行层尚未接入，当前不读写数据库记录。合并范围及修复见 [A 版本合并说明](docs/a-merge-notes.md)。
...
│   ├── json-plan-protocol.md # C++ 编译器到 Java 引擎的 JSON 协议
...
│   ├── parser/parser.cpp     # A：递归下降、五类语句、表达式和深度限制
...
├── app/main.cpp              # A 调试入口：标准输入 SQL → Token 和 AST 展示
...
├── tests/
│   ├── lexer/lexer_tests.cpp # A：6 组词法测试（含合并回归）
│   ├── parser/parser_tests.cpp # A：9 组语法测试（含合并回归）
│   ├── catalog/catalog_tests.cpp # B：4 个模式/快照行为用例
│   ├── semantic/semantic_tests.cpp # B：33 个语义行为用例
│   ├── test_support.hpp      # 测试断言和手工 AST 辅助，不属于产品 API
│   ├── planner/plan_tests.cpp # B：16 个计划结构与打印用例
│   ├── optimizer/            # B：21 组优化测试及独立参考求值器
│   └── integration/scaffold_smoke.cpp # 12 个真实 SQL → Plan 兼容性用例
...
团队原始交付副本保留在 `A version/DBcompiler/` 供溯源，已从主工程 Git 跟踪和构建中排除。
...
| 规则优化 | 保持操作符位置、文法及优先级契约 | 常量折叠、布尔化简、恒真 Filter 消除和等价性测试 |
...
| minisql_plan_json | SQL 编译为 protocolVersion 1 JSON 执行计划 | frontend + backend |
| lexer_tests / parser_tests | A 的模块测试，仅 BUILD_TESTING 开启时构建 | frontend |
| scaffold_smoke | 真实 SQL→计划的 12 个联调用例，仅 BUILD_TESTING 开启时构建 | frontend + backend |
...
./build/minisql_plan_json < demo.sql
```

默认开启 BUILD_TESTING；`-DBUILD_TESTING=OFF` 可关闭测试目标。
以上可执行路径适用于默认单配置构建；多配置生成器使用对应 Debug/Release 子目录。
minisql 从标准输入读取 SQL，打印 Token Stream 和 AST；可使用 `./build/minisql < input.sql`。
不接收命令行文件参数，多余参数返回 2；词法/语法错误返回 1，成功返回 0。
...
该脚本用 c++ 和 ar 分别构建 A/B 静态库，验证十二个公共头文件可独立包含，
再编译运行前端空输入检查、四种示例、A 的 15 组模块测试、12 个联调用例、
B 原有的 53 个行为用例及新增的 21 组优化测试。产物位于 `build/direct/`，不会覆盖
CMake 构建文件；可通过 CXX/AR 环境变量指定工具路径。

当前环境未安装 CMake；验证使用上述脚本，CMake/CTest 路径尚未在本机验证。
本次直接编译使用 `-Wall -Wextra -Wpedantic -Werror`；A 的词法/语法、12 个兼容性用例、
B 原有 53 个行为用例和新增 21 组优化测试全部通过。A 合并时另验证了 CLI 正常输入、词法错误、语法错误和错误参数。
测试包含快照隔离、失败注册无副作用、名称大小写、列重排、表达式类型、错误位置及深度边界。
新增用例包含 UPDATE 旧值引用、重复赋值、DELETE 条件、修改计划行标识、
输出模式、外部非连续列 ID、无效绑定结果诊断和确定性打印。
新增联调用例验证真实 SQL 的优先级、源码范围、CRLF、转义、整数边界和五类计划。
...
优化测试及其编译链路还通过了 UBSan 检查（21 组，未报告未定义行为）；复现命令见
...
2. A 按 token → Lexer::run → Parser::statement → 分层表达式函数阅读，
   新的边界修复及构造深度元数据见 [合并说明](docs/a-merge-notes.md)。

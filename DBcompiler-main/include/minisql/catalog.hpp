...
struct IndexId { std::uint64_t value; };
...
public:
    ...
    virtual std::shared_ptr<const IndexSchema> findIndex(
        std::string_view normalized_name) const = 0; // 不存在返回 nullptr。
    virtual IndexId nextIndexId() const noexcept = 0;
};
